...
GYGAX = {  # APPENDIX_A
'''
This dictionary keys are strings roman numerals which match the table 
lables in the 1st Edition, Advanced Dungeons & Dragons, Dungeon Master's Guide.
("DMG") Each value is a complete table which contains the following:

Tables:
An array containing the "TABLE NAME", die size, and array of "results". Die size
refers to the die that is to be rolled to determine a result. Most of the tables
are based on a twenty sided die, some are percentile and I thin there might be
a couple d6's thrown in there.

The results array is a list of tuples and each tuple contains the following:
Integers representing the low and high die rolls for a particular result, a 
short string name of the result, an optinional long triple quoted string with 
longer description or notes attempting to capture all the information from the
DMG; followed by another array.  The last array is a series of commands that
can be run automatically if the player is using this table to actually build a
dungeon.
'''
    "I": ["PERIODIC CHECK", 20, [  # results array 
